...
<div class="page-breadcrumb">
    <div class="row">
        ...
        <div class="customize-input float-right">
                @canany(['car_exchange_category-create'])
                    <a class="btn waves-effect waves-light btn-rounded btn-outline-primary pull-right" href="{{ route('car_exchange_category.create') }}">Add New</a>
                @endcanany
            </div>
        </div>
    </div>
    ...
    $(document).ready(function() {
                if ($('#data-table').length > 0) {
                    datatable = $('#data-table').DataTable({
                            ...
                            "aaSorting": [],
                                ...
                                "ajax": {
                                    "url": "{{ route('car_exchange_category') }}",
                                    "type": "POST",
                                    "dataType": "json",
                                    "data": {
                                        _token: "{{csrf_token()}}"
            ...
            function change_status(object) {
                ...
                if (confirm('Are you sure?')) {
                    $.ajax({
                        "url": "{!! route('car_exchange_category.change.status') !!}",
                        "dataType": "json",
                        "type": "POST",
                        "data": {
                            id: id,
