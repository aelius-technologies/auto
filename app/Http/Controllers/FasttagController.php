...
class FasttagController extends Controller{
    ...
    /** index */
        public function index(Request $request){
            if($request->ajax()){
                $data = Fasttag::orderBy('id' , 'desc')->get();
                ...
                return Datatables::of($data)
                        ...
                        ->addColumn('action', function($data){
                            ...
                            if(auth()->user()->can('fasttags-view')){
                                $return .= '<a href="'.route('fasttag.view', ['id' => base64_encode($data->id)]).'" class="btn btn-default btn-xs">
                                                <i class="fa fa-eye"></i>
                                            </a> &nbsp;';
                            }   
                            
                            if(auth()->user()->can('fasttags-edit')){
                                $return .= '<a href="'.route('fasttag.edit', ['id' => base64_encode($data->id)]).'" class="btn btn-default btn-xs">
        ...
        public function insert(Request $request){
            ...
            try {
                ...
                if ($last_id) {
                    ...
                    DB::commit();
                    return redirect()->route('fasttag')->with('success', 'Record inserted successfully');
                } else {
                    DB::rollback();
    ...
    /** view */
        public function view(Request $request){
            ...
            $data = Fasttag::where(['id' => $id])->first();
            return view('fasttag.view')->with(['data' => $data]);
        }
    /** view */
    ...
    /** edit */
        public function edit(Request $request){
            $id = base64_decode($request->id);
            $data = Fasttag::where(['id' => $id])->first();
            return view('fasttag.edit')->with(['data' => $data]);
        }
    /** edit */ 
        ...
        public function update(Request $request){
            ...
            try {
                ...
                if ($update) {
                    ...
                    DB::commit();
                    return redirect()->route('fasttag')->with('success', 'Record updated successfully');
                } else {
                    DB::rollback();
    ...
    /** change-status */
        public function change_status(Request $request){
            ...
            if (!empty($data)) {
                $update = Fasttag::where(['id' => $id])->update(['status' => $request->status, 'updated_at' => date('Y-m-d H:i:s'), 'updated_by' => auth('sanctum')->user()->id]);
                if ($update) {
